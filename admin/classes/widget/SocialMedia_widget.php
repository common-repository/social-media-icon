...
class SocialMedia_widget extends WP_Widget {
	...
	function __construct() {
		parent::__construct(
			'SocialMedia_widget_id', // Base ID
			__( 'Social Media Widget'), // Name
			array( 'description' => __( 'Display Social Media icon in widget area.' ), ) // Args
			);
	}
	...
	public function widget( $args, $instance ) {
		...
		if(!empty($buttonId)){
			...
			echo do_shortcode( '[Social_Media id='.$buttonId.']' );		
		}else{
			echo "<p>No Social Media Group Found !!!!.</p>";
		}
	...
	public function form( $instance ) {
		...
		<select id="<?php echo $this->get_field_id( 'Shortcode' ); ?>" name="<?php echo $this->get_field_name( 'Shortcode' ); ?>[]" style="width: 100%;" multiple="multiple">
			...
			if($all_button->have_posts())
			{
				...
				echo "<option>Sorry! No Social Media Group Found.</option>";
			}
